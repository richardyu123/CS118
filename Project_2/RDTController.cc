...
void RDTController::Receive(std::string& str_buffer, size_t num_bytes) {
    ...
    while(receive_base == received.front().num && !received.empty()) {
        vector<char> data = received.front().pkt.GetData();
        size_t size = min(num_bytes - curr, static_cast<size_t>(distance(
                        data.begin() + bytes_remaining, data.end())));
        for (auto iter = data.begin() + bytes_remaining; iter != data.begin() +
             bytes_remaining + size; iter++) {
            *str_iter = *iter;
            str_iter++;
        }
        curr += size;
    ...
    while (curr < num_bytes) {
        ...
        if (seq_num <= receive_base + parameters::WINDOW_SIZE - 1 && seq_num >=
                receive_base) {
            bool retrans = false;
